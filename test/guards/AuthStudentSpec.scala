...
class AuthStudentSpec extends AsyncFunSpec with Matchers with BeforeAndAfter with DatabaseSupport {
  ...
  describe("isAuthorized") {
    it("should return true for a student") {
      AuthStudent.isAuthorized(student, authenticator)(FakeRequest()) map (r => r should be(true))
    }

    it("should return false for an educator") {
      AuthStudent.isAuthorized(educator, authenticator)(FakeRequest()) map (r => r should be(false))
    }
  }
