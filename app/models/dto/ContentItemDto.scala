...
object ContentItemDto {
  ...
  def construct(id: Option[Int], packageId: Int, imageUrl: Option[String], name: String, content: String) =
    new ContentItemDto(id = id, packageId = packageId, imageUrl = imageUrl, name = name, content = content)

  def deconstruct(dto: ContentItemDto): Option[(Option[Int], Int, Option[String], String, String)] = dto match {
    case ContentItemDto(id: Option[Int], packageId: Int, imageUrl: Option[String], name: String, content: String, _, _, _) =>
      Some(id, packageId, imageUrl, name, content)
  }

  def formConstruct(id: Option[Int], packageId: Int, imageUrl: Option[String], name: String, content: String, enabled: Option[Boolean]) =
    new ContentItemDto(id = id, packageId = packageId, imageUrl = imageUrl, name = name, content = content, enabled = enabled.getOrElse(true))

  def formDeconstruct(dto: ContentItemDto): Option[(Option[Int], Int, Option[String], String, String, Option[Boolean])] = dto match {
    case ContentItemDto(id: Option[Int], packageId: Int, imageUrl: Option[String], name: String, content: String, enabled: Boolean, _, _) =>
      Some(id, packageId, imageUrl, name, content, Some(enabled))
  }
  ...
  class ContentItemsTable(tag: Tag) extends Table[ContentItemDto](tag, "content_items") {
    ...
    def packageId: lifted.Rep[Int]           = column[Int]("package_id")
    def imageUrl: lifted.Rep[Option[String]] = column[String]("image_url").?
    ...
    def * : ProvenShape[ContentItemDto] =
      (id, packageId, imageUrl, name, content) <>
  ...
  /**
    ...
    */
  implicit val getContentItemResult: GetResult[ContentItemDto] = GetResult(
    r =>
      ContentItemDto(
        Some(r.nextInt),
        r.nextInt,
  ...
  /**
    * implicit converter to coerce direct sql query into data object
    */
  implicit val getSomeContentItemResult: GetResult[Some[ContentItemDto]] = GetResult(
    r =>
      Some(
        ContentItemDto(
          Some(r.nextInt),
  ...
  /**
    ...
    */
  implicit val serializer: OFormat[ContentItemDto] = Json.format[ContentItemDto]
}
