...
object ContentAssignedDto {
  ...
  def deconstruct(dto: ContentAssignedDto) = dto match {
    case ContentAssignedDto(id: Option[Int], name: String, examDate: LocalDate, active: Boolean, ownerId: Option[Int], _, _, _) =>
      Some(id, name, examDate, active, ownerId)
  }

  def formConstruct(id: Option[Int],
                    name: String,
                    examDate: LocalDate,
                    active: Boolean,
                    ownerId: Option[Int],
                    enabled: Option[Boolean],
                    cohorts: Option[List[CohortDto]],
                    packages: Option[List[ContentPackageDto]]) =
    new ContentAssignedDto(id = id,
                           name = name,
                           examDate = examDate,
                           active = active,
                           ownerId = ownerId,
  ...
  def formDeconstruct(dto: ContentAssignedDto) = dto match {
    case ContentAssignedDto(id: Option[Int],
                            name: String,
                            examDate: LocalDate,
                            active: Boolean,
                            ownerId: Option[Int],
                            ...
                            cohorts: Option[List[CohortDto]],
                            packages: Option[List[ContentPackageDto]]) =>
      Some(id, name, examDate, active, ownerId, Some(enabled), cohorts, packages)
  }
  ...
  class ContentAssignedTable(tag: Tag) extends Table[ContentAssignedDto](tag, "content_assigned") {
    ...
    def examDate: lifted.Rep[LocalDate]  = column[LocalDate]("exam_date")
    def active: lifted.Rep[Boolean]      = column[Boolean]("active")
    def ownerId: lifted.Rep[Option[Int]] = column[Option[Int]]("owner_id")
    def pk: PrimaryKey                   = primaryKey("PRIMARY", id)

    def * : ProvenShape[ContentAssignedDto] =
      (id, name, examDate, active, ownerId) <>
  ...
  /**
    ...
    */
  implicit val getCAResult: GetResult[ContentAssignedDto] = GetResult(
    r =>
      ContentAssignedDto(
        Some(r.nextInt),
        r.nextString(),
  ...
  /**
    * implicit converter to coerce direct sql query into data object
    */
  implicit val getSomeCAResult: GetResult[Some[ContentAssignedDto]] = GetResult(
    r =>
      Some(
        ContentAssignedDto(
          Some(r.nextInt),
  ...
  /**
    ...
    */
  implicit val serializer: OFormat[ContentAssignedDto] = Json.format[ContentAssignedDto]
}
