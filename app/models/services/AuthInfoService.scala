...
class AuthInfoService extends AuthInfoRepository {
  override def find[T <: AuthInfo](loginInfo: LoginInfo)(implicit tag: ClassManifest[T]): Future[Option[T]] = ???
  ...
  override def remove[T <: AuthInfo](loginInfo: LoginInfo)(implicit tag: ClassManifest[T]): Future[Unit] = ???
}
